...
/**
 * Computes multivariate gaussian functions
 * @param (x, y) x, y of landmark from map
 * @param (ux, uy) x, y of landmark from detection
 * @param (sx, sy) standard deviations
 * @output Multivariate Gaussian function value
 */
inline double multivariateGaussian(double x, double y, double ux, double uy, double sx,
                            double sy) {
  double expTermx = ((x - ux) * (x - ux)) / (2. * sx * sx);
  double expTermy = ((y - uy) * (y - uy)) / (2. * sy * sy);
  return (1. / (2. * M_PI * sx * sy)) * exp(-(expTermx + expTermy));
}
